...
Now that we have cleaned the data we can now create a couple of simple plots to find the most common words. Let's use a word cloud to determine which words have the highest frequency in the dataset. We need to use the wordcloud package to acomplish this easily. 

```{r}
wordcloud(clean_corpus, max.words = 65, colors=brewer.pal(6, "Set2"))
```

We can see from the word cloud most common words are things like *the*, *like*, and *will*.
