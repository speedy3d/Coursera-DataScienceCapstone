...
Now we will determine how many usable words are in each file. We will do this by using the stri_stats_latex funtion that is found within the stringi library. This will also show us the number of characters.

```{r}
stri_stats_latex(dataset_blogs)
stri_stats_latex(dataset_news)
stri_stats_latex(dataset_twitter)
```

## Analyzing the Data

Next we will dive a little deeper to determine try and gleen some more useful information from these files. Namely we will try to determine and plot the most common words and word pairs (bigrams).

```{r}
qplot(stri_count_words(dataset_news))
```
